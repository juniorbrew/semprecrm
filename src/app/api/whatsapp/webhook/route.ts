...
async function processMessage(
  ...
  // Reactions short-circuit here — they aren't messages. We still need
  // the contact + conversation so the reaction can be attached to the
  // right thread, but we never insert into `messages`, never bump
  // unread_count, never update last_message_text. Done before
  // parseMessageContent so the media-URL fetch is skipped.
  ...
  // Parse message content based on type (verifies media with Meta and
  // builds the proxy URL). `mediaType` is only informational — the
  // messages table has no media_type column.
