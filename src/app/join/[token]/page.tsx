...
export default function JoinPage() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error ||
              t('You are already in another account. Sign in with a different email to join this one.'),
          );
        } else {
          toast.error(payload.error || t('Failed to accept invitation'));
        }
        setAccepting(false);
        return;
      }
      toast.success(t('Welcome to the team'));
  ...
  const handleSignOutAndRetry = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('Could not sign out. Try refreshing the page.'));
      setSigningOut(false);
    }
  }, [t]);
    ...
    return (
      <Card className="w-full max-w-md border-border bg-card">
        <CardContent className="flex flex-col items-center gap-3 py-12">
          ...
          <p className="text-sm text-muted-foreground">{t('Verifying invitation…')}</p>
        </CardContent>
      </Card>
    );
  }
          ...
          {peek.reason === 'server_error' ? (
            <>
              ...
              <Link href="/signup">
                ...
                >
                  {t('Create a new account instead')}
                </Button>
              </Link>
            </>
          ) : (
    ...
    <CardHeader className="items-center text-center">
      ...
      <CardDescription className="text-muted-foreground">
        ...
        <span className="inline-flex items-center gap-1 text-foreground">
          ...
          {t(ROLE_LABEL[peek.role])}
        </span>
        {t('. Link valid until')}{' '}
        {new Date(peek.expires_at).toLocaleDateString(language, {
          year: 'numeric',
          month: 'short',
          day: 'numeric',
        })}
        ...
        <Card className="w-full max-w-md border-border bg-card">
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-center text-xs text-muted-foreground">
              {t('Accepting moves your login into')}{' '}
              <span className="text-muted-foreground">{peek.account_name}</span>
              {t('. Your empty personal account from signup will be cleaned up.')}
            </p>
          </CardContent>
        </Card>
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-popover-foreground">
                ...
                {t('Unable to join')} {peek.account_name} {t('with this account')}
              </DialogTitle>
            ...
            <DialogFooter className="bg-popover border-border">
              ...
              >
                {signingOut ? (
                  ...
                  t('Sign out & use a different email')
                )}
              </Button>
            </DialogFooter>
          </DialogContent>
