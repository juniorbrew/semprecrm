  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-border bg-popover text-popover-foreground sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">{t("Custom fields")}</DialogTitle>
...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (isDuplicate(name)) {
      toast.error(t('A field with this name already exists.'));
      return;
    }
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        `${t('Delete field')} "${field.field_name}"? ${t('This also removes its stored value on every contact. This cannot be undone.')}`
      )
    ) {
      return;
    }
  ...
  return (
    <li className="flex items-center gap-2 px-3 py-2">
      <Input
        ...
        aria-label={`${t('Rename field')}: ${field.field_name}`}
