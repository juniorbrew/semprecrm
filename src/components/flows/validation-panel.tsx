...
import { translateIssueMessage } from "./issue-messages";
  ...
  const body = (
    <>
      ...
      <span className="min-w-0 flex-1">
        ...
        {translateIssueMessage(issue.message, language, t)}
      </span>
    </>
  );
  ...
  if (issue.node_key && onJump) {
    return (
      <button
        ...
        aria-label={`${t("Jump to node")} ${issue.node_key}`}
      >
        {body}
      </button>
    );
