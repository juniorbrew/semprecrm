  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-border bg-card">
      <header className="border-b border-border px-5 py-4">
        <h2 className="text-sm font-semibold text-foreground">{t('Pipeline Value')}</h2>
      ...
      <div className="flex flex-1 flex-col p-5">
        {loading || !data ? (
          ...
          <>
            <Donut data={data} currency={currency} label={t('Pipeline value by stage')} />
...
function Donut({
  ...
  return (
    <div className="flex items-center justify-center">
      <svg viewBox={`0 0 ${size} ${size}`} className="h-48 w-48" role="img" aria-label={label}>
        {/* background ring */}
        ...
        {/* center label — "Total" reads the same in pt-BR and en-US */}
        <text
          x={cx}
          y={cy - 6}
          textAnchor="middle"
